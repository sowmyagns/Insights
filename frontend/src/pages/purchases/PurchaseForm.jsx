...
export default function PurchaseForm() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [vendorRes, companyRes] = await Promise.allSettled([getVendors(), getCompanySettings()]);
        if (cancelled) return;
        ...
        if (editId) {
          ...
          const meta = doc.meta || {};
          setPurchaseType(meta.purchase_type || "purchase_invoice");
          ...
          setForm((f) => ({
            ...
            invoice_number: doc.document_number || f.invoice_number,
        ...
        if (!cancelled) {
          addToast(apiErrorMessage(err, "Failed to load purchase"), "error");
          if (editId) navigate("/purchases");
        }
      } finally {
    ...
    try {
      const payload = {
        ...
        meta: {
          ...
          purchase_discount: showPurchaseDiscount ? purchaseDiscount : null,
          ...
          <section className="border-b border-[#d0d0d8] p-4 lg:border-b-0 lg:border-r">
            <div className="mb-4 flex flex-wrap gap-5 border-b border-[#d0d0d8] pb-4">
              <label className="inline-flex cursor-pointer items-center gap-2 text-[13px] text-[#1a1a1f]">
                <input
                  type="radio"
                  name="purchaseType"
                  checked={purchaseType === "purchase_invoice"}
                  onChange={() => setPurchaseType("purchase_invoice")}
                />
                Purchase Invoice
              </label>
              <label className="inline-flex cursor-pointer items-center gap-2 text-[13px] text-[#1a1a1f]">
                <input
                  type="radio"
                  name="purchaseType"
                  checked={purchaseType === "bill_of_supply"}
                  onChange={() => setPurchaseType("bill_of_supply")}
                />
                Bill of Supply
              </label>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="block">
                <FieldLabel>Purchase Invoice No.</FieldLabel>
                <SoftInput
