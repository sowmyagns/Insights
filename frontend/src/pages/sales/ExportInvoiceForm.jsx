...
export default function ExportInvoiceForm() {
  ...
  const [exportDetailsOpen, setExportDetailsOpen] = useState(true);
  const [exportCustomFields, setExportCustomFields] = useState([]);
  const [exportCustomFieldOpen, setExportCustomFieldOpen] = useState(false);
    ...
    (async () => {
      try {
        ...
        setForm((f) => ({
          ...
          transport_mode: inv.transport_mode || f.transport_mode || "Road",
        ...
        if (!cancelled) {
          ...
          navigate("/sales/export-invoices");
        }
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        custom_fields: [
          ...(exportType ? [{ label: "Export Type", value: exportType }] : []),
          ...(iecNo ? [{ label: "IEC No.", value: iecNo }] : []),
          ...exportCustomFields.map((f) => ({ label: f.label, value: f.value })),
          ...customFields.map((f) => ({ label: f.label, value: f.value })),
        ],
        notes: [
          ...(exportType ? [`Export Type: ${exportType}`] : []),
          ...(iecNo ? [`IEC No.: ${iecNo}`] : []),
          ...exportCustomFields.map((f) => `${f.label}: ${f.value}`),
      ...
      if (isEdit) {
        ...
        addToast("Export invoice updated");
      } else {
        ...
        addToast("Export invoice created");
      }
      navigate("/sales/export-invoices");
    } catch (err) {
      console.error(err);
    ...
    >
      ...
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-[#e4e4ea] bg-white px-5 py-3.5">
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => navigate("/sales/export-invoices")}
        ...
        <div className="space-y-3">
          ...
          <section className="overflow-hidden rounded-xl border border-[#d0d0d8] bg-white">
            <SectionHeader
              ...
              onToggle={() => setExportDetailsOpen((v) => !v)}
            />
            {exportDetailsOpen ? (
              <div className="space-y-4 border-t border-[#ececf0] p-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="block">
                    <FieldLabel accent>Export Type</FieldLabel>
                    <SoftSelect
                      value={exportType}
                ...
                {exportCustomFields.map((field) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setExportCustomFields((rows) => rows.filter((x) => x.id !== field.id))
                      }
                      className="rounded p-1 text-[#9a9aa5] hover:bg-[#f0f0f4] hover:text-[#e11d48]"
                      aria-label={`Remove ${field.label}`}
                    >
                ...
                <button
                  type="button"
                  onClick={() => setExportCustomFieldOpen(true)}
      ...
      />
      <AddCustomFieldModal
        open={exportCustomFieldOpen}
        onClose={() => setExportCustomFieldOpen(false)}
        onSave={(field) => setExportCustomFields((rows) => [...rows, field])}
      />
      <AddBankAccountModal
        open={bankModalOpen}
